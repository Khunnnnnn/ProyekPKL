            ...
            <table id="datatables" class="table table-bordered table-striped table-sm" style="font-size: 15px; position: relative;">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    @if($admin->id_status==1)
                    <form action="{{ url('post/updateGuru', $admin->id ) }}" class="d-inline" method="POST">
                      @csrf
                      @method('put')
                      <button class="btn btn-secondary btn-sm">Nonaktif</button>
                    </form>
                    @endif
                    @if($admin->id_status==2)
                    <form action="{{ url('post/update', $admin->id ) }}" class="d-inline" method="POST">
                      @csrf
                      @method('put')
                    ...
                    <div class="modal fade" id="editAdmin{{ $admin->id }}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                      <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content bg-warning">
                          ...
                          <div class="modal-body text-white" style="background-color: #ffa200;">
                            <form action="{{route('guru.update',$admin->id)}}" method="POST">
                              @csrf
                              @method('put')
                              <div class="form-group">
                                <label for="nama">Nama Pengguna</label>
                    ...
                    <form action="{{route('guru.destroy',$admin->id)}}" method="POST" class="d-inline">
                      @csrf
                      @method('DELETE')
